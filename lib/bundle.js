	...
	expandDependencies : function( bundleRegistry ) {
		...
		expandedDependencies = _.map( expandedDependencies, function( bundleName ) {
			if( _.isUndefined( bundleRegistry[ bundleName ] ) ) {
				var errMsg = "Could not find bundle in bundle map: " + bundleName + " for bundle " + _this.name;
...
function createRegistryForDirectory( directory, rootDir, dirOptions, mode ) {
	...
	//set remote files as remote=true property
	bundleOptions.remoteFiles = _.map(bundleOptions.remoteFiles, function(remoteFile){
		if (_.isObject(remoteFile)){
			remoteFile["remote"] = true;
		}
		return remoteFile;
	});

	//filter remote files based on environment
	bundleOptions.remoteFiles = _.filter(bundleOptions.remoteFiles, function(remoteFile){
		if(_.isString(remoteFile)){
			return true;
		} else if(_.isObject(remoteFile)){
			if(mode === "dev" && _.has(remoteFile, "devMode") && remoteFile["devMode"] === false){
				return false;
			} else if(mode === "prod" && _.has(remoteFile, "prodMode") && remoteFile["prodMode"] === false){
				return false;
			}
			return true;
		} else {
			console.log("Warning: Couldnt identify remote file="+JSON.stringify(remoteFile));
		}
	});

	if( bundleOptions.prioritizeFlattenedDirectories ) {
		fileDependencies = fileDependencies.concat( bundleOptions.remoteFiles );
	}
	else {
		fileDependencies = bundleOptions.remoteFiles.concat( fileDependencies );
	}
	...
	_.each( fileDependencies, function( filePath ) {
		var file;
		if( File.isCDNFile( filePath ) ) {
			file =  File.createAndRegister( { src : filePath, keepSeparate : true } );
		}
		else {
			file = File.createAndRegister( { src : walker.fullPath( filePath ), keepSeparate : false } );
