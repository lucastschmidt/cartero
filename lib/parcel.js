...
Parcel.createRegistry = function( dirs, bundleRegistry, mode ) {
	...
	_.each( parcelRegistry, function( parcel ) {
		if(_.isArray(parcel.includedParcels)){
			parcel.includedParcels = _.map(parcel.includedParcels, function(includedParcel){
				if(!_.isObject(parcelRegistry[ includedParcel ])){
					throw new Error("Parcel "+includedParcel+" couldnt be found");
				}
				includedParcel = parcelRegistry[ includedParcel ];
				return includedParcel;
			});
		}
	} );
...
Parcel.prototype.getFilesToServe = function( bundlesAlreadyInParcel, mode, forceKeepSeparate ) {
	...
	if(_.isArray(this.includedParcels)){
		_.each(this.includedParcels, function(includedParcel){
			files = _.union( files, includedParcel.getFilesToServe( bundlesAlreadyInParcel, mode, false ) );
		});
	}
	...
	_.each( pageFiles, function( fileName ) {
		...
		if( !_.isNull( carteroRequiresMatches ) ) {
			try {
				...
				if(_.isFunction(dirOptions.findIncludedDependencies)){
					var includedDependencies = dirOptions.findIncludedDependencies(path.join(dirOptions.destDir, parcelDirectory), pageFileContents, fileName);
					if(_.isArray(includedDependencies)){
						_.map(includedDependencies, function(includedDep){
							if(_.isString(includedDep)){
								return path.join(parcelDirectory, includedDep);
							} else {
								console.log("Warning: Included Dependency not detected="+JSON.stringify(includedDep));
								return includedDep;
							}
						});
						dependencies = dependencies.concat(includedDependencies);
					}
				}
			}
			catch( e ) {
		...
		} else if (_.isFunction(dirOptions.findExtendedParcel)){
			extendsParcel = dirOptions.findExtendedParcel(path.join(dirOptions.destDir, parcelDirectory), pageFileContents, fileName);
			if(_.isString(extendsParcel)){
				extendsParcel = path.join(parcelName, "..", extendsParcel);
			}
		}

		// find parcel extended based on includes
		var includedParcels = null;
		if(_.isFunction(dirOptions.findIncludedParcels)){
			includedParcels = dirOptions.findIncludedParcels(path.join(dirOptions.destDir, parcelDirectory), pageFileContents, fileName);
			if(_.isArray(includedParcels)){
				includedParcels = _.map(includedParcels, function(includedDep){
					if(_.isString(includedDep)){
						return path.join(parcelName, "..", includedDep);
					} else {
						console.log("Warning: Included Parcel not detected="+JSON.stringify(includedDep));
						return includedDep;
					}
				});
			}
		...
		parcelRegistry[ parcelName ] = new Parcel( {
			...
			includedParcels: includedParcels
		} );
	} );
